...
class ProductControllerTest {
	...
	void getProductDetailSuccess() throws Exception {
		...
		ProductDetailResponseDto mockResponseDto = ProductDetailResponseDto.builder()
				...
				.preferredTradeLocation(tradeLocationDto)
				.viewCount(52)
				.isLiked(true)
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/products/{productId}", productId)
				...
				.andExpect(jsonPath("$.data.category.mainCategoryId").value(1L))
				.andExpect(jsonPath("$.data.category.subCategoryId").value(9L))
				...
				.andExpect(jsonPath("$.data.preferredTradeLocation.latitude").value(37.497952))
				.andExpect(jsonPath("$.data.preferredTradeLocation.longitude").value(127.027619))
				.andExpect(jsonPath("$.data.preferredTradeLocation.address").value("서울 강남구 강남역"))
